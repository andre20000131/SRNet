...
from models.modules.noise_main import noise_dict
...
class SRNet(nn.Module):
    ...
    def forward(self, x):
        ...
        noise = noise_dict[w]
        #out.to('cuda')
        #self.stylegan_decoderdotstyle_conv1dotweight.to('cuda')
        noise = noise.to('cuda:0')
        #print(noise.is_cuda,out.is_cuda,self.stylegan_decoderdotstyle_conv1dotweight.is_cuda)
        ...
        # conv2
        style = self.stylegan_decoderdotstyle_convsdot1dotmodulated_convdotmodulation(latent[:, i + 1]).view(1, 1, 512,
                                                                                                             1, 1)
        ...
        out = torch.cat([out_same, out_sft], dim=1)
        # conv2
        style = self.stylegan_decoderdotstyle_convsdot3dotmodulated_convdotmodulation(latent[:, i + 1]).view(1, 1, 512,
                                                                                                             1, 1)
        ...
        # conv2
        style = self.stylegan_decoderdotstyle_convsdot5dotmodulated_convdotmodulation(latent[:, i + 1]).view(1, 1, 512,
                                                                                                             1, 1)
        ...
        out = torch.cat([out_same, out_sft], dim=1)
        # conv2
        style = self.stylegan_decoderdotstyle_convsdot7dotmodulated_convdotmodulation(latent[:, i + 1]).view(1, 1, 512,
                                                                                                             1, 1)
        ...
        out = torch.cat([out_same, out_sft], dim=1)
        # conv2
        style = self.stylegan_decoderdotstyle_convsdot9dotmodulated_convdotmodulation(latent[:, i + 1]).view(1, 1, 256,
                                                                                                             1, 1)
        ...
        out = torch.cat([out_same, out_sft], dim=1)
        # conv2
        style = self.stylegan_decoderdotstyle_convsdot11dotmodulated_convdotmodulation(latent[:, i + 1]).view(1, 1, 128,
                                                                                                              1, 1)
        ...
        out = torch.cat([out_same, out_sft], dim=1)
        # conv2
        style = self.stylegan_decoderdotstyle_convsdot13dotmodulated_convdotmodulation(latent[:, i + 1]).view(1, 1, 64,
                                                                                                              1, 1)
