...
def save_img_with_ratio(image_path, image):
    #align_ratio = (2 ** 16 - 1) / image.max()
    #np.save(alignratio_path, align_ratio)
    image = np.clip(image,0,1)
    uint8_image_gt = np.round(image * 255).astype(np.uint8)
    cv2.imwrite(image_path, uint8_image_gt)
    return None

def generate_paths(folder, name):
    id = name
    image_path = os.path.join('../results', id+'.png')
