...
def main():
    ...
    input_folder = '/data/sr_5000'
    save_folder = '/data/sr_5000_crop'

    n_thread = 20
    crop_sz = 512   # crop size
    step = 256  # crop stride
    thres_sz = 48
...
def worker(path, save_folder, crop_sz, step, thres_sz, compression_level):
    ...
    for x in h_space:
        for y in w_space:
            ...
            cv2.imwrite(
                os.path.join(save_folder, img_name.replace('.png', '_{}.png'.format(index))),
